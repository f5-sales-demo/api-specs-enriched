---
title: HTTP 负载均衡器增强
description: HTTP 负载均衡器模式的丰富元数据
sidebar:
  order: 2
i18n:
...
HTTP 负载均衡器模式的丰富元数据。有关扩展定义，请参阅 [OpenAPI 扩展](../development/#openapi-extensions)。
...
- 配置文件中为 `lb_type`（`config/minimum_configs.yaml` 中的简写形式）
...
2. 在 `config/discovered_defaults.yaml` 中添加配置
3. 重新运行丰富处理管道以应用扩展
...
- [健康检查增强](./healthcheck-enhancements/) - 健康检查模式丰富处理（包含 `x-f5xc-recommended-oneof-variant`）
- [源池增强](./originpool-enhancements/) - 源池模式丰富处理
