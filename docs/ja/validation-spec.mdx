...
信頼スコア付きのフィールド名パターンベースのバリデーションルール。
...
拡充された OpenAPI 仕様は、以下の拡張機能を使用してバリデーションメタデータを埋め込みます。
...
複数のソースが制約を提供する場合、調整の優先順位は以下のとおりです。

1. **既存** - 元の OpenAPI 仕様にある制約（最高優先度）
2. **ディスカバリー** - ライブ API ディスカバリーからの制約
...
- [CLAUDE.md](../../CLAUDE.md) - AI アシスタントの手順
