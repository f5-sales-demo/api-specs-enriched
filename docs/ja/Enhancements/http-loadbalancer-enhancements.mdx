---
title: HTTP ロードバランサーの拡張機能
description: HTTP ロードバランサースキーマの拡充メタデータ
sidebar:
  order: 2
i18n:
...
HTTP ロードバランサースキーマの拡充メタデータです。拡張定義については [OpenAPI Extensions](../development/#openapi-extensions) を参照してください。
...
- 設定ファイルでは `lb_type`（`config/minimum_configs.yaml` の短縮形）
...
- [Healthcheck の拡張機能](./healthcheck-enhancements/) - Healthcheck スキーマの拡充（`x-f5xc-recommended-oneof-variant` を含む）
