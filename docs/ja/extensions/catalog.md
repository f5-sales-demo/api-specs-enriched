...
- **Applied at:** info
- **Purpose:** ライブ API ディスカバリーパスが実行されたときのタイムスタンプ。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{"type": "string", "format": "date-time"}`
...
- **Applied at:** schema
- **Purpose:** このスキーマにマッピングされる Terraform リソースタイプ名。
- **Consumers:** Terraform
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** 生成されたドキュメントおよび UI に表示するデフォルト値。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/field_metadata_enricher.py
...
- **Applied at:** schema property
- **Purpose:** クライアントがプロパティを省略した場合にサーバーが割り当てるデフォルト値。
- **Consumers:** multiple
- **Value type:** string
- **Value schema:** `{}`
...
- **Applied at:** schema property
- **Purpose:** あるフィールドが別のフィールドの設定を必要とするクロスフィールド依存関係を文書化します。
...
- **Applied at:** operation
- **Purpose:** ディスカバリー中にこのオペレーションで実測された応答時間。
- **Consumers:** multiple
- **Value type:** object
- **Value schema:** `{"type": "object"}`
- **Injected by:** scripts/utils/discovery_enricher.py
...
- **Applied at:** info
- **Purpose:** このドメインがサポートする名前付きユースケース。
- **Consumers:** multiple
- **Value type:** array
- **Value schema:** `{"type": "array", "items": {"type": "string"}}`
- **Injected by:** scripts/merge_specs.py
...
- **Applied at:** info
- **Purpose:** UI でこのドメインをレンダリングする際に使用するアイコン識別子。
- **Consumers:** Web UI
- **Value type:** string
- **Value schema:** `{"type": "string"}`
- **Injected by:** scripts/merge_specs.py
