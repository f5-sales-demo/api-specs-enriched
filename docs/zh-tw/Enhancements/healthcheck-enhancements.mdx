...
健康檢查相關結構描述的擴充元資料。擴充定義請參閱 [OpenAPI 擴充](../development/#openapi-extensions)。

## 已擴充的結構描述

### 模式

所有符合 `healthcheck.*SpecType` 的結構描述均會收到擴充：
...
標記有 `x-f5xc-server-default: true` 的欄位，其 `default` 值會在請求中省略時由 F5 XC API 伺服器自動套用。
...
包含互斥欄位群組（OneOf）的結構描述會附帶 `x-f5xc-recommended-oneof-variant`，用以指出最常使用的變體。此元資料根據 F5 XC 主控台預設值及常見使用模式，識別在多個選項存在時的典型選擇。
...
這些供應商擴充會加入標準 OpenAPI 結構描述，以傳達 F5 XC 特定的預設行為。
