---
title: Sistema di Metadati per Vincoli
description: >-
  Metadati completi sui vincoli per la generazione deterministica con IA e la
  validazione
sidebar:
  order: 2
i18n:
...
3. [Tipi di vincolo](#constraint-types)
...
7. [Integrazione con il sistema di discovery](#discovery-integration)
...
- Esempio: `admin@example.com`, `api+webhook@example.com`
...
2. **Catalogo degli errori**: Messaggi di errore API che indicano violazioni dei vincoli
   - "name must be 1-63 characters" → minLength/maxLength
   - "port must be between 1 and 65535" → minimum/maximum

3. **Validazione delle risposte**: Risposte API andate a buon fine che dimostrano valori validi
