...
│   - Release pinned  │     │   - Release artifacts       │
└─────────────────────┘     └─────────────────────────────┘
```
...
```text
Download (release) → Enrich → Normalize → Canonical merge → Lint → Serve
     │              │          │         │       │
     ▼              ▼          ▼         ▼       ▼
 Source specs   Branding    Fix refs   Domain docs  Spectral
...
| `docs/specifications/api/` | Generated domain specs  | Tracked release input                |
...
```bash
# Set credentials
export XCSH_API_URL="https://your-tenant.console.ves.volterra.io/api"
...
2. The latest stable upstream release is resolved and its asset digest verified
3. Pipeline processes and enriches specs
4. The committed build version is calculated before artifact generation
5. A release commit and tag are created for that exact generated tree
...
```bash
# Start from the fetched protected branch
git fetch --prune
git switch --no-track -c feature/123-my-change origin/main
...
git add config/example.yaml scripts/example.py tests/test_example.py
...
test -n "${XCSH_API_TOKEN:-}" && echo "XCSH_API_TOKEN is set"
test -n "${XCSH_API_URL:-}" && echo "XCSH_API_URL is set"
```
...
Downloads are pinned by the immutable six-field `.github_release` receipt:
`version`, `tag_name`, `published_at`, `asset_name`, `asset_size`, and
`asset_sha256`. The asset name and SHA-256 identify the downloaded upstream bytes;
the committed release version identifies the generated build. Neither identity is
derived from an ETag or from a mutable branch.

```bash
# The release workflow calculates VERSION, then stamps that exact identity.
python -m scripts.stamp_release_version docs/specifications/api --version "$VERSION"
```
...
| `make download`       | Download the pinned stable release |
...
| Variable         | Purpose                  |
| ---------------- | ------------------------ |
| `XCSH_API_URL`   | F5 XC tenant API URL     |
| `XCSH_API_TOKEN` | API authentication token |

### Key Files

| File                                  | Purpose                                      |
| ------------------------------------- | -------------------------------------------- |
| `.github_release`                     | Immutable six-field upstream release receipt |
...
**Note**: Release generation supplies and commits the exact build version; the
upstream receipt independently identifies the source archive.
