...
| ------------- | ------------------------------------ | --------------- | ----------- |
| `public_name` | Origin server with public DNS name   | `dns_name`      | Emitted     |
| `public_ip`   | Origin server with public IP address | `ip` (IPv4)     | Emitted     |
...
## Site-Dependent Origin Server Types

The generated artifact currently emits these variants and their measured
`site_locator` dependency metadata:

| Type             | Description                                        | Required Fields                                           | Status                |
| ---------------- | -------------------------------------------------- | --------------------------------------------------------- | --------------------- |
| `private_ip`     | Origin server with private/public IP and site info | `ip`, `site_locator`, `network_choice`, `snat_pool`       | Emitted               |
| `private_name`   | Origin server with DNS name and site info          | `dns_name`, `site_locator`, `network_choice`, `snat_pool` | Emitted               |
| `k8s_service`    | Kubernetes service with site info                  | `service_name`, `site_locator`, `network_choice`          | Emitted               |
| `consul_service` | HashiCorp Consul service with site info            | `service_name`, `site_locator`, `network_choice`          | Emitted               |

### Current Emitted Metadata

The emitted contract records:

1. **Site locator requirements** for site-dependent variants
2. **Network-choice constraints** where measured
3. **SNAT-pool fields** supplied by the upstream schema
4. **Cross-resource dependency metadata** for site and virtual-site references

The source is `config/minimum_configs.yaml` and the versioned OpenAPI artifact,
not a future implementation recommendation.
