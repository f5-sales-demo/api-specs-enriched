...
Download the versioned `f5xc-api-specs-v<VERSION>.zip` asset from the matching
GitHub release and read `domains/validation.json`. Consumers must verify the bundle
digest against the publication receipt embedded in that immutable release; Pages
and `raw/main` URLs are not release identities.
