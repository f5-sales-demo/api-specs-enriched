---
title: 오리진 풀 개선 사항
description: 오리진 풀 스키마에 대한 보강 메타데이터
sidebar:
  order: 3
i18n:
...
origin_pool 관련 스키마에 대한 보강 메타데이터입니다. 확장 정의는 [OpenAPI 확장](../development/#openapi-extensions)을 참조하십시오.

## 보강된 스키마

### 패턴

`origin_pool.*SpecType`과 일치하는 모든 스키마는 보강을 적용받습니다:
...
`x-f5xc-server-default: true`로 표시된 필드는 요청에서 생략될 경우 F5 XC API 서버가 해당 `default` 값을 적용합니다.
