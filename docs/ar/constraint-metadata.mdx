...
description: >-
  بيانات وصفية شاملة للقيود من أجل توليد الذكاء الاصطناعي الحتمي والتحقق من
  الصحة
sidebar:
  order: 2
i18n:
  sourceHash: ca1a17c63719
  translator: machine
---
...
يوفر امتداد `x-f5xc-constraints` معرفة حتمية بقيود التحقق من صحة الحقول، مما يتيح ما يلي:
...
| *(مفاتيح القيود)* | متنوعة | مفاتيح القيود الخاصة بالنوع مسطّحة على المستوى الأعلى (مثل `minLength` و`maxItems` و`minimum`) |
...
- مثال: `admin@example.com`، `api+webhook@example.com`
...
تُضبط العلامة البوليانية `deterministic` عندما:
- تكون درجة الثقة >= 0.9
...
2. **كتالوج الأخطاء**: رسائل أخطاء API التي تشير إلى انتهاكات القيود
   - "name must be 1-63 characters" ← minLength/maxLength
   - "port must be between 1 and 65535" ← minimum/maximum

3. **التحقق من الاستجابات**: استجابات API الناجحة التي تُظهر قيمًا صحيحة
...
### الأنماط عالية الثقة (0.95-0.99)

| النمط | تعبير اسم الحقل | نوع القيد | مثال |
|---------|------------------|-----------------|---------|
...
- **المشكلات**: الإبلاغ عن مشكلات دقة القيود على https://github.com/f5-sales-demo/api-specs-enriched/issues
