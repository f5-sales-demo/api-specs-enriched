...
def _resource_version_schema(*, required: bool = False) -> dict:
    schema = {
        "type": "object",
        "properties": {"resource_version": {"type": "string"}},
    }
    if required:
        schema["required"] = ["resource_version"]
    return schema


def _exact_output_graph() -> list[dict]:
    schemas = {
        **{f"Fixture{index:03d}GetResponse": _resource_version_schema() for index in range(411)},
        **{f"Fixture{index:03d}ReplaceRequest": _resource_version_schema() for index in range(339)},
    }
...
def test_exact_output_resource_version_accounting_is_required():
    specs = _exact_output_graph()

    assert pipeline._require_expected_output_resource_versions(specs) == (411, 339)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Fixture410GetResponse",
            r"expected=\(411, 339\), measured=\(410, 339\)",
        ),
        (
            "Fixture338ReplaceRequest",
            r"expected=\(411, 339\), measured=\(411, 338\)",
        ),
    ],
)
def test_output_resource_version_accounting_rejects_under_count(
    schema_name: str,
    message: str,
):
    specs = _exact_output_graph()
    del specs[0]["components"]["schemas"][schema_name]

    with pytest.raises(RuntimeError, match=message):
        pipeline._require_expected_output_resource_versions(specs)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "OverflowGetResponse",
            r"expected=\(411, 339\), measured=\(412, 339\)",
        ),
        (
            "OverflowReplaceRequest",
            r"expected=\(411, 339\), measured=\(411, 340\)",
        ),
    ],
)
def test_output_resource_version_accounting_rejects_over_count(
    schema_name: str,
    message: str,
):
    specs = _exact_output_graph()
    specs[0]["components"]["schemas"][schema_name] = _resource_version_schema()
...
@pytest.mark.parametrize(
    "mutate",
    [
        lambda schemas: schemas["Fixture000GetResponse"]["properties"].pop("resource_version"),
        lambda schemas: schemas.setdefault("UnrelatedResponse", _resource_version_schema()),
    ],
)
def test_output_resource_version_accounting_rejects_misdeclared_location(mutate):
    specs = _exact_output_graph()
    mutate(specs[0]["components"]["schemas"])

    with pytest.raises(RuntimeError, match="resource_version declaration mismatch"):
        pipeline._require_expected_output_resource_versions(specs)


def test_output_resource_version_accounting_rejects_required_token():
    specs = _exact_output_graph()
    schemas = specs[0]["components"]["schemas"]
    schemas["Fixture000ReplaceRequest"] = _resource_version_schema(required=True)

    with pytest.raises(RuntimeError, match="requires resource_version"):
        pipeline._require_expected_output_resource_versions(specs)


def test_output_resource_version_accounting_rejects_non_string_token():
    specs = _exact_output_graph()
    declaration = specs[0]["components"]["schemas"]["Fixture000GetResponse"]["properties"][
        "resource_version"
    ]
    declaration["type"] = "integer"

    with pytest.raises(RuntimeError, match="resource_version is not a string schema"):
        pipeline._require_expected_output_resource_versions(specs)
